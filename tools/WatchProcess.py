...
class WATCH:
    def __init__(self):
        self.log=logs.Log()
        self.receiver=['zhangxiaokang@huohua.cn']
    def watch_process(self):
        self.dba_db = MysqlPool.Mysql('dba')

        rows=self.dba_db.getAll('select id,process_name,run_command,auto_run from `dba`.watch_process where is_del=3 ')
        info=[]
        for row in rows:
            ...
            if len(info)==0 :
                ...
                msgBody='{0} 程序退出，请检查 '.format(row["process_name"])
                ...
                if   row["auto_run"]==1 :
                    ...
                    msgBody='{0}  将尝试拉起。  '.format(row["process_name"] )
                    dingsender.send(msgBody)
